...
class Session {
	...
	static function session_start_dj($name = 'BestWebs', $limit = 0, $path = '/', $domain = '.indocrm.space', $secure = null) {
		if (ini_set('session.use_only_cookies', 1) === FALSE) {		 //// Forces sessions to only use cookies. ////
			echo '<script>window.location= "'.LOGIN_URL.'/error.html?Please enable browser cookies to use this application then reload page.";</script>';
	        exit;
		}
