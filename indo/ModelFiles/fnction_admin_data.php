...
################################################## MENU
    ...
    function getArray_categoryOfferProducts($catId, $start, $limit){
        $solution = $this->dbConn->prepare("SELECT
                b.ul_title brand,
                p.ul_title title,
                p.ul_permalink permalink,
                ...
                WHERE m.ul_category_id = :id
                ORDER BY p.ul_price_discount_client DESC, p.ul_status DESC
                LIMIT $start, $limit");
        $solution->execute(array(':id'=>$catId));
        if ($products = $solution->fetchAll(PDO::FETCH_ASSOC)) {
            return $products;
        }else{
            return $this->noProduct('s');
