        ...
        <div class="col-lg-12">
            <div class="box dark">
                <header>
                    ...
                    <h5 id="pageTitle">Register Product Warranty & Customer</h5>
                    <div class="toolbar">
                        <ul class="nav">
                            <li class="dropdown">
                    ...
                    <form class="ajax-form" action="" method="post" enctype="multipart/form-data">
                        <div class="row">
                            <div class="col-xs-12 panel-group" id="">
                                    ...
                                    <div id="collapseOne" class="panel-collapse collapse in">
                                        <div class="panel-body">
                                            ...
                                            <div class="row">
                                              ...
                                              <div class="col-xs-6 form-group">
                                                <label>CRN</label>
                                                <input type="text" class="form-control" value="<?php echo $customer['id']; ?>" name="customer_id" id="customer_id" readonly="" />
                                              </div>
                                            </div>
                                                <?php
                                            }else{
                                            ...
                                            <div class="row">
                                              ...
                                              <div class="col-sm-9 col-xs-12 form-group" id="locationField">
                                            ...
                                            <div class="row">
                                              ...
                                              <div class="col-sm-3 col-xs-6 form-group">
                                                ...
                                                <input type="tel" class="form-control" placeholder="Area pin Code" name="form[city_pin]" id="postal_code" pattern="[0-9]{6}" maxlength="6" onchange="this.setCustomValidity(this.validity.patternMismatch ? '6 Digit Area Pin Code' : '');" title="Please Enter Correct Area Pin Code, It will be used for contact" data-toggle="tooltip" required/>
                                              </div>
                                              <div class="col-sm-3 col-xs-6 form-group">
                                                <label>City <sup>*</sup></label>
                                                <input type="text" class="form-control" placeholder="Landmark" name="city" id="administrative_area_level_2" pattern="[0-9a-zA-Z .,-]{2,100}" maxlength="100" onchange="this.setCustomValidity(this.validity.patternMismatch ? 'Please Enter City, Minimum 2 & Maximum 100 and only - , . allowed' : '');" />
                                              </div>
                                              <div class="col-sm-3 col-xs-6 form-group">
                                                <label>State</label>
                                                <input type="text" class="form-control" placeholder="Landmark" id="administrative_area_level_1" pattern="[0-9a-zA-Z .,-]{2,100}" maxlength="100" onchange="this.setCustomValidity(this.validity.patternMismatch ? 'Please Enter State, Minimum 2 & Maximum 100 and only - , . allowed' : '');" />
                                              </div>
                                              <div class="col-sm-3 col-xs-6 form-group">
                                                <label>Land Mark</label>
                                                <input type="text" class="form-control" placeholder="Landmark" name="form[landmark]" id="landmark" pattern="[0-9a-zA-Z .,-]{2,100}" maxlength="100" onchange="this.setCustomValidity(this.validity.patternMismatch ? 'Please Enter Landmark, Minimum 2 & Maximum 100 and only - , . allowed' : '');" />
                                              </div>
                                            </div>
                                            <div class="row">
                                            ...
                                            <div class="row">
                                              ...
                                              <div class="row">
                                                  ...
                                                  <div class="col-xs-4 col-lg-2 form-group">
                                                    ...
                                                    <input type="number" class="form-control" name="product[quantity]" id="quantity" min="1" max="250" value="1" placeholder="in Pcs."/>
                                                  </div>
                                ...
                                <div class="panel panel-primary dealerPanel">
                                    <div class="panel-heading">
                                        <h4 class="panel-title">
    ...
    $(function(){
        ...
        $('#productSearch').on("change", function(){
            ...
            performAjax(varsArray, " ", function(status, msg){
                if(status === "success"){
                    if(! msg) {
                        showResponse("danger", "Product with Code <b>"+id+"</b> is not in our Database <a href='/BestWebs?module=product&mode=add&type=new' target='_blank'> Click Here to add </a> or Try another.", 10000);
                        $("#productSearch").val("");
                        return;
                    }

                    $("#productBrand").val(msg.brand).trigger('chosen:updated').change();
                    $("#productCat").val(msg.category).trigger('chosen:updated').change();
                    $("#product").val(msg.product).trigger('chosen:updated').change();
                    $("#productModel").val(msg.id).trigger('chosen:updated').change();
                }
            }, true);
        });
        ...
        <?php
            ...
            if ($type != "new" && $level >= 8) {
                $id = (int) $type;
                if(isset($_GET['entity']) && $_GET['entity'] == 'customer-product'){
                    echo "var id = $id,
                          details = ".json_encode($function->getDetail_customerProduct($id)).";";
                    ?>
                    if(details){
                        // $("#collapseThree").slideDown('slow');
                        $('.customerPanel').remove();
                        $('.productPanel').append('<input type="hidden" name="product[customer_product_id]" value="'+id+'"><input type="hidden" name="product[customer_id]" value="'+details.id+'">');

                        $("#productBrand").val(details.brand).trigger('chosen:updated').change();
                        $("#productCat").val(details.productCat).trigger('chosen:updated').change();
                        $("#product").val(details.product).trigger('chosen:updated').change();
                        $("#productModel").val(details.product_id).trigger('chosen:updated').change();
                        $("#productModelText").val(details.model);
                        $("#warranty_status").val(details.warranty).trigger('chosen:updated');
                        if(details.purchase_date) $("#purchase_date").val(details.purchase_date.split('/').reverse().join('-'));
                        $("#quantity").val(details.quantity);
                        $("#productCode").val(details.productCode);
                        $("#productSearch").val(details.product_id);

                        $("#purchased_from").val(details.purchased_from).trigger('chosen:updated').change();
                        $("#dealer").val(details.dealer);
                        $("#dealer_address").val(details.dealer_address);
                        $("#dealer_pin").val(details.dealer_pin).trigger('keyup');

                        $('#pageTitle').html('Edit Customer #'+details.code+"'s Product");
                        $('#submit').val("Update Customer Product");
                    }
                    <?php
                }else{
                    echo "var id = $id,
                          details = ".json_encode($function->getDetail_customer($id)).";";
                    ?>
                    if(details){
                        $('.dealerPanel').remove();
                        $('.productPanel').remove();
                        $('.customerPanel').append('<input type="hidden" name="form[customer_id]" value="'+id+'">');
                        $("#name").val(details.customer);
                        $("#address").val(details.address);
                        $("#postal_code").val(details.pin);
                        $("#postal_code").trigger('change');
                        $("#landmark").val(details.landmark);
                        $("#mobile").val(details.mobile);
                        $("#alternate_mobile").val(details.alternate_mobile);
                        $("#email").val(details.email);
                        $("#administrative_area_level_2").val(details.district);
                        $("#administrative_area_level_1").val(details.state);
                        $("#company").val(details.company);

                        $('#pageTitle').html('Edit Customer #'+details.code);
                        $('#submit').val("Update Customer");
                    }
                    <?php
                }
            }
...
<script>
  ...
  <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyB-5-M6XkSzfkRPY6RKgYe86hfJATvqy-U&libraries=places&callback=initAutocomplete"></script>