...
class Session {
	...
	static function session_start_dj($name = SESSION_NAME, $limit = SESSION_COOKIE_TIME, $path = '/', $domain = '', $secure = HTTPS_SECURED) {
		if (ini_set('session.use_only_cookies', 1) === FALSE) {		 //// Forces sessions to only use cookies. ////
			header('Location: http://error.bestwebs.in?error=Please enable browser cookies to use this application then reload page.');
            echo '<script>window.location= "http://error.bestwebs.in?error=Please enable browser cookies to use this application then reload page.";</script>';
	        exit;
		}
