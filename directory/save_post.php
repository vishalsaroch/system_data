...
<?php
	...
	$image4 = addslashes(file_get_contents($_FILES["image4"]["tmp_name"]));
		...
		//$sql = "INSERT INTO `post` (`title`, `price`, `categories`, `sub-categories`,`condition`,`ad-for`) VALUES ('$title', '$price', '$Categories', '$sub_Categories','$Condition')";
			
		if ($conn->query($sql) === TRUE) {
			echo "ad posted successfully";
		   echo "<script>location='db-payment.php'</script>";
				exit();
