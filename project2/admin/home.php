...
function handleIt333(){
	var urlkey3333 = "./markHandled333.php?packid333="+packid333;
	//alert(urlkey3);
	$.ajax({
				url: urlkey3333,
				method: "GET",
...
function markCustomer555(){
	var urlkey3555 = "./markCustomer555.php?packid555="+packid555;
	//alert(urlkey3);
	$.ajax({
				url: urlkey3555,
				method: "GET",
						...
						if($result->num_rows>0){
							...
							while($row = $result->fetch_assoc()){
								if($row["isHandled3"]==1)
									...
									echo "<tr onclick='showcard555(this);'><td>" . $row["sno"]. "</td><td>" . $row["name"]. "</td><td>" . $row["activities"]."</td><td>" . $row["destination"]."</td><td class='text-right'>" . $row["date"]."</td><td class='text-right'>" . $row["contact"]. "</td></tr>";
