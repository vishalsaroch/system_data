...
if($type == "tagged"){
    if ($level > 7) {
        $action = '
            <a href="javascript:void(0);" class="btn btn-success btn-circle btn-line btn-action" data-action="jobsheet" title="Print Job-Sheet"><i class="icon-book"></i></a>
            <a href="javascript:void(0);" class="btn btn-primary btn-circle btn-line btn-action" data-action="edit-ticket" title="Edit"><i class="icon-edit"></i></a>
        ...
        $action = '<a href="javascript:void(0);" class="btn btn-success btn-circle btn-line btn-action" data-action="jobsheet" title="Print Job-Sheet"><i class="icon-book"></i></a>';
    }
    $status = " com.ul_status = 3 " ;
    ...
    $status = " com.ul_status = 1 " ;
    $type = "Without Spare Replaced Closed";
}elseif($type == "replaced"){
    if ($level > 7) {
        $action = '
            <a href="javascript:void(0);" class="btn btn-success btn-circle btn-line btn-action" data-action="jobsheet" title="Print Job-Sheet"><i class="icon-book"></i></a>
            <a href="javascript:void(0);" class="btn btn-primary btn-circle btn-line btn-action" data-action="edit-ticket" title="Edit"><i class="icon-edit"></i></a>
    ...
    $status = " com.ul_status = 4 " ;
    $type = "Spare Replaced Closed";
}elseif($type == "pending"){
    $action = '
        <a href="javascript:void(0);" class="btn btn-success btn-circle btn-line btn-action" data-action="jobsheet" title="Print Job-Sheet"><i class="icon-book"></i></a>
        <a href="javascript:void(0);" class="btn btn-success btn-circle btn-line btn-action" data-action="add-job" title="Add Job"><i class="icon-plus-sign"></i></a>
        <a href="javascript:void(0);" class="btn btn-danger btn-circle btn-line btn-action" data-action="resendCustomer"  title="Resend Customer SMS"><i class="icon-envelope-alt"></i></a>
        ';
    if ($level > 7) {
        $action .= '<a href="javascript:void(0);" class="btn btn-info btn-circle btn-line btn-action" data-action="technician" title="Assign Technician"><i class="icon-github-sign"></i></a>
            <br>
            <a href="javascript:void(0);" class="btn btn-danger btn-circle btn-line btn-action" data-action="resendCenter"  title="Resend Center SMS"><i class="icon-envelope"></i></a>
            <a href="javascript:void(0);" class="btn btn-warning btn-circle btn-line btn-action" data-action="view-otp" title="View OTP"><i class="icon-eye-open"></i></a>
            <a href="javascript:void(0);" class="btn btn-primary btn-circle btn-line btn-action" data-action="edit-ticket" title="Edit"><i class="icon-edit"></i></a>
            <a href="javascript:void(0);" class="btn btn-danger btn-circle btn-line btn-action" data-action="delete"  title="Delete"><i class="icon-trash"></i></a>
        ';
    }elseif ($level > 4) {
        ...
        <div class="col-lg-12">
            <div class="box DARK data-table-box">
                <header>
                    ...
                    <h5><?php echo ucwords($type); ?> Tickets</h5>
                    <div class="toolbar">
                        <ul class="nav">
                ...
                <div id="box-1" class="accordion-body collapse in body">
                    <table class="table table-bordered table-hover wide-table data-table">
                        ...
                        <tbody>
                            <?php
                                if ($results = $function->getArray_tickets($start, $results, $where, $whereArray)) {
...
<script>
    $(function(){
        $("body").on('click', '#updateTechnician' ,function(e) {
            var user = $('[name="center_user"]').val(),
                code = $('#updateTechnicianCode').text(),
                contact = $('[name="center_user"] option:selected').attr('data-contact'),
                name = $('[name="center_user"] option:selected').attr('data-technician'),
                action = "updateTechnician",
                entity = $(this).attr("data-id"),
                varsArray = {"adminAjax":action, "entity":entity, "id":{"code":code, "user":user, "name":name, "contact":contact}};
            if(! user){$('[name="center_user"]').before("<label><sup>Required</sup></label>"); return false;} ;
