...
(function ($) {
	...
	$.fn.ajaxChosen = function (ajaxOptions, options, chosenOptions) {
	    ...
	    function processValue(e) {
			var field = $(this),
				entity = field.closest('.chosen-container').siblings('.chzn-select-ajax').attr('data-entity'),
				q = field.val();
			//don't fire ajax if...
			...
			if ($.isArray(ajaxOptions.data)) {
				...
				ajaxOptions.data = ajaxOptions.data.concat({ name: 'q', value: q, 'entity':entity, "adminAjax":"fetchDependentSelect"});
			} else {
				//hash
				if (!('data' in ajaxOptions)) {
					ajaxOptions.data = {};
				}
				$.extend(ajaxOptions.data, { data: {q: q, 'entity':entity, "adminAjax":"fetchDependentSelect"} });
			}
